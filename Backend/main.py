...
async def create_mock_settings(settings: MockSettings):
    global mock_id
    # Validación explícita para evitar duplicados
    for mock in mock_settings_storage:
        if mock["ruta"] == settings.ruta and mock["metodo"].upper() == settings.metodo.upper():
            raise HTTPException(
                status_code=400,
                detail="Mock with this route and method already exists"
            )
